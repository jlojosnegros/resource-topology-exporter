...
func AddFile(watcher *fsnotify.Watcher, notifyFilePath string, allowExisting bool) (FilterEvent, error) {
	if notifyFilePath == "" {
		// nothing to do
		return FilterEverything, nil
	}

	err := EnsureNotifyFilePath(notifyFilePath, allowExisting)
	if err != nil {
...
// filesystem. Return error if the creation file, or if the path already exists.
// The `allowExisting` relaxes this check partially, allowing to reuse a file
// previously created with this function.
func EnsureNotifyFilePath(notifyFilePath_ string, allowExisting bool) error {
	...
	if info, err := os.Stat(notifyFilePath); err == nil {
		if !allowExisting {
			return fmt.Errorf("cannot use %q: already exists", notifyFilePath)
		}
